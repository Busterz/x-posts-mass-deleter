...
function buildAcceptLanguageString() {
	...
	if (!languages || languages.length === 0) {
		return "en-US,en;q=0.8"; // Default value if nothing is available
	}
...
async function fetch_tweets(cursor, retry = 0) {
	...
	if (delete_options["old_tweets"] == false) {
		...
		feature = `&features=%7B%22rweb_tipjar_consumption_enabled%22%3Atrue%2C%22responsive_web_graphql_exclude_directive_enabled%22%3Atrue%2C%22verified_phone_label_enabled%22%3Afalse%2C%22creator_subscriptions_tweet_preview_api_enabled%22%3Atrue%2C%22responsive_web_graphql_timeline_navigation_enabled%22%3Atrue%2C%22responsive_web_graphql_skip_user_profile_image_extensions_enabled%22%3Afalse%2C%22communities_web_enable_tweet_community_results_fetch%22%3Atrue%2C%22c9s_tweet_anatomy_moderator_badge_enabled%22%3Atrue%2C%22articles_preview_enabled%22%3Atrue%2C%22tweetypie_unmention_optimization_enabled%22%3Atrue%2C%22responsive_web_edit_tweet_api_enabled%22%3Atrue%2C%22graphql_is_translatable_rweb_tweet_is_translatable_enabled%22%3Atrue%2C%22view_counts_everywhere_api_enabled%22%3Atrue%2C%22longform_notetweets_consumption_enabled%22%3Atrue%2C%22responsive_web_twitter_article_tweet_consumption_enabled%22%3Atrue%2C%22tweet_awards_web_tipping_enabled%22%3Afalse%2C%22creator_subscriptions_quote_tweet_preview_enabled%22%3Afalse%2C%22freedom_of_speech_not_reach_fetch_enabled%22%3Atrue%2C%22standardized_nudges_misinfo%22%3Atrue%2C%22tweet_with_visibility_results_prefer_gql_limited_actions_policy_enabled%22%3Atrue%2C%22rweb_video_timestamps_enabled%22%3Atrue%2C%22longform_notetweets_rich_text_read_enabled%22%3Atrue%2C%22longform_notetweets_inline_media_enabled%22%3Atrue%2C%22responsive_web_enhance_cards_enabled%22%3Afalse%7D&fieldToggles=%7B%22withArticlePlainText%22%3Afalse%7D`
	}
	else {
	...
	if (!response.ok) {
		if (response.status === 429) {
			var rateLimitReset = parseInt(response.headers.get('X-Rate-Limit-Reset',-1))
			const currentTime = Math.floor(Date.now() / 1000);
			const resetDate = new Date(rateLimitReset * 1000).toLocaleString();
			const timeUntilReset = rateLimitReset - currentTime;
...
function tweetFound(obj) {
	console.log(`Found tweet: ${obj['legacy']['full_text']}. RT? ${obj['legacy']['retweeted']} Liked? ${obj['legacy']['favorited']}`)
}
...
function findTweetIds(obj) {
	function recurse(currentObj) {
		...
		var pathToTweetContent = {};
		if (currentObj.hasOwnProperty('content') && currentObj['content'].hasOwnProperty('itemContent')
			&& currentObj['content']['itemContent'].hasOwnProperty('tweet_results') && currentObj['content']['itemContent']['tweet_results'].hasOwnProperty('result') && currentObj['content']['itemContent']['tweet_results']['result'].hasOwnProperty('legacy'))
		{
			pathToTweetContent = currentObj['content']['itemContent']['tweet_results']['result'];
		}
		else if(currentObj.hasOwnProperty('content') && currentObj['content'].hasOwnProperty('items'))
		{
			pathToTweetContent = currentObj['content']['items'];
		}

		if(Array.isArray(pathToTweetContent)){
			for(const el of pathToTweetContent){
				if(el.hasOwnProperty('item') && el['item'].hasOwnProperty('itemContent')
					&& el['item']['itemContent'].hasOwnProperty('tweet_results') && el['item']['itemContent']['tweet_results'].hasOwnProperty('result') && el['item']['itemContent']['tweet_results']['result'].hasOwnProperty('legacy')){
						var pathToLegacy = el['item']['itemContent']['tweet_results']['result'];
						if(pathToLegacy.hasOwnProperty('user_id_str') && check_tweet_owner(pathToLegacy, user_id) && check_filter(pathToLegacy)){
							tweets_to_delete.push(pathToLegacy['legacy']['id_str']);
							tweetFound(pathToLegacy);
						}
				}
			}
		} else if(typeof pathToTweetContent === 'object' && check_tweet_owner(pathToTweetContent, user_id) && check_filter(pathToTweetContent)){
			tweets_to_delete.push(pathToTweetContent['legacy']['id_str']);
			tweetFound(pathToTweetContent);
		}
	}
